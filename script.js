...
let sessionInterval = null; // Monitor de sesión
...
// ─── SOLUCIÓN SESIÓN FANTASMA: Monitor Automático ───
function iniciarMonitorSesion() {
  ...
  }, 5000); // Verifica cada 5 segundos en segundo plano
}

function forzarCierreSesion() {
  if (sessionInterval) clearInterval(sessionInterval);
  const isLoginVisible = document.getElementById("login-section").style.display !== "none";
  if (!isLoginVisible) {
    alert("Tu sesión ha expirado por inactividad. Por seguridad, vuelve a iniciar sesión para continuar.");
    logout();
  } else {
    borrarSesion();
  }
...
document.getElementById("barrido-form").addEventListener("submit", async function (e) {
  ...
  // SOLUCIÓN: Chequeo estricto de sesión justo antes de procesar el guardado
  ...
  const datos = {
    usuario:     String(sesionActiva.usuario), // SOLUCIÓN: Usa la sesión validada
    fecha:       (() => {
                   const now = new Date();
...
async function proy_guardar() {
  ...
  const usuario = sesionActiva.usuario; // SOLUCIÓN: Usa la sesión validada
  ...
  try {
    ...
    // Si la sesión caducó silenciosamente en proy_leerFilas, reasignamos el usuario aquí por seguridad a todas las filas
    const filasVerificadas = filas.map(f => ({ ...f, usuario: usuario }));
    
    const { error: insErr } = await supabaseClient.from('proyeccion').insert(filasVerificadas);
    ...
    document.getElementById("proy-asesor-view").style.display  = "none";
    document.getElementById("proy-preview-view").style.display = "block";
    proy_renderPreview(filasVerificadas);
